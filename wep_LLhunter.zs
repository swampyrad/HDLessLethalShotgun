...
//		bobb.y=clamp(bobb.y,-8,8);
	...
	action void A_CheckPocketSaddles(){
		...
		if(!countinv("HDLLShellAmmo"))invoker.weaponstatus[0]&=~HUNTF_FROMPOCKETS;
	}
	...
	hold:
		...
		goto ready;
	fire:
		LLSG A 0 A_JumpIf(invoker.weaponstatus[HUNTS_CHAMBER]==2,"shoot");
		LLSG A 1 A_WeaponReady(WRF_NONE);
		LLSG A 0 A_Refire();
		goto ready;
	shoot:
	...
	altfire:
	chamber:
		LLSG A 0 A_JumpIf(invoker.weaponstatus[0]&HUNTF_ALTHOLDING,"nope");
	...
	racked:
		...
		LLSG F 0 A_JumpIf(invoker.weaponstatus[HUNTS_CHAMBER],"racked");
		LLSG F 0{
			int rld=0;
			if(pressingreload()){
				rld=1;
	...
	rackreload:
		...
		LLSG F 0 A_JumpIf(!(invoker.weaponstatus[0]&HUNTF_FROMPOCKETS),"rackloadone");
	...
	chamberauto:
		LLSG A 1 A_Chamber();
		LLSG A 1 A_JumpIf(invoker.weaponstatus[0]&HUNTF_CANFULLAUTO&&invoker.weaponstatus[HUNTS_FIREMODE]==2,"ready");
		LLSG A 0 A_Refire();
		goto ready;
	flash:
		SHTF B 1 bright{
	...
	reloadstarthand:
		...
		LLSG C 0 A_JumpIf(invoker.weaponstatus[0]&HUNTF_FROMPOCKETS,"reloadpocket");
	reloadfast:
	...
	reloadend:
		...
		LLSG CBA 3;
		LLSG A 0 A_JumpIf(invoker.weaponstatus[0]&HUNTF_HOLDING,"nope");
		goto ready;
